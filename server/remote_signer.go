...
// SignBYOCJobRequestInput is the JSON body accepted by POST /sign-byoc-job.
// Fields mirror byoc.BYOCJobSigningInput, which is what gets bound into the
// V1 signature payload via byoc.FlattenBYOCJob.
...
// SignBYOCJobRequest signs a BYOC job using the V1 binary format
// (byoc.FlattenBYOCJob). The orch's verifyJobCreds reconstructs the same bytes
// and validates the returned sig against the returned sender.
